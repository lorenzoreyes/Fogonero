...
"""4 main functions to handle the cycle of portfolio management
  1 portfolioMonitor to update and suggets rebalance allocaiton
  2 DepositOrWithdraw suggest new composition based on a capital change
...
def PortfolioMonitor(dataframe):
  ...
  portfolio['priceToday'] = (yahoo.download(stocks,period="1d",interval="1m",prepost=True)['Adj Close'].fillna(method='ffill')).tail(1).T
...
def DepositOrWithdraw(dataframe, change):
  ...
  portfolio['PnLpercent'] = portfolio['notionalToday'] / portfolio['notionalStart']
  portfolio['PnLpercentEach'] = portfolio['priceToday'] / portfolio['pricePaid']
  portfolio['DepositOrWithdraw'] = float(change)
  # En nuevo nominal sumamos el resultado obtenido mas el remanente liquido para reinvertir, siendo nuestro total disponible
...
def AdjustRisk(portfolio):
  ...
  listado = list(portfolio.iloc[:,0].values)
  df = yahoo.download(listado,period="1y",interval="60m")['Adj Close'].fillna(method='ffill')
  data = df 
...
def portfolioRiskUpdated(portfolio):
  """Provide your portfolio composition to apply
     the update of risk by Component-Value-at-Risk"""
  update = AdjustRisk(portfolio)
  cartera = pd.DataFrame(portfolio)
  df = pd.DataFrame(index=cartera.iloc[:,0])
  df['nominal'] = cartera['nominal'].values
  df['pricePaid'] = cartera['price'].values  
  ...
  df['oldLiquidity'] = cartera['liquid'].values
